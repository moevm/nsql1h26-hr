...
class CandidateService:
    ...
    async def create_candidate(
        self, candidate_data: CandidateCreate
    ) -> CandidateResponse:
        if candidate_data.vacancy_id:
            vacancy = await self.vacancy_repo.get_vacancy_by_id(
                candidate_data.vacancy_id
            )
            if not vacancy:
                raise AppError(
                    "Cannot find vacancy with given UUID", status.HTTP_400_BAD_REQUEST
                )
        if candidate_data.test_task_id:
            test_task = await self.test_task_repo.get_test_task_by_id(
                candidate_data.test_task_id
            )
            if not test_task:
                raise AppError(
                    "Cannot find test task with given UUID", status.HTTP_400_BAD_REQUEST
                )
            if (
                candidate_data.vacancy_id
                and str(candidate_data.vacancy_id) != test_task["vacancy_id"]
            ):
                raise AppError(
                    "Test task is not for given vacancy", status.HTTP_400_BAD_REQUEST
                )
        if candidate_data.status != CandidateStatus.NEW:
            raise AppError("Candidate status must be NEW", status.HTTP_400_BAD_REQUEST)
