...
class VacancyService:
    ...
    async def create_vacancy(self, vacancy_data: VacancyCreate) -> VacancyResponse:
        ...
        if (
            ...
            or vacancy_data.closed_at is not None
        ):
            raise AppError("Cannot create CLOSED vacancy", status.HTTP_400_BAD_REQUEST)
    ...
    async def patch_vacancy(
        self, vacancy_id: UUID, vacancy_data: VacancyPatch
    ) -> VacancyResponse:
        if (
            vacancy_data.status == VacancyStatus.OPEN
            and vacancy_data.closed_at is not None
        ):
            raise AppError("Invalid vacancy status", status.HTTP_400_BAD_REQUEST)
        if vacancy_data.status is None and vacancy_data.closed_at is not None:
            vacancy_data.status = VacancyStatus.CLOSED
        if (
            vacancy_data.status == VacancyStatus.CLOSED
            and vacancy_data.closed_at is None
        ):
