...
async def test_create_vacancy(hr_client):
    title = "Vacancy 1"
...
async def test_get_vacancy_by_id_ok(hr_client):
    title = "Vacancy 1"
    description = "Test Description"
    response = await hr_client.post(
        "/vacancies", json={"title": title, "description": description}
    ...
    response = await hr_client.get(f"/vacancies/{vacancy_id}")
    assert response.status_code == 200
    got_data = response.json()
    assert got_data == data


async def test_get_vacancy_by_id_bad_uuid(hr_client):
    response = await hr_client.get("/vacancies/bad-id")
    assert response.status_code == 422


async def test_patch_vacancy_ok(hr_client):
    title = "Vacancy 1"
    description = "Test Description"
    response = await hr_client.post(
        "/vacancies", json={"title": title, "description": description}
...
async def test_patch_vacancy_date(hr_client):
    title = "Vacancy 1"
    description = "Test Description"
    response = await hr_client.post(
        "/vacancies", json={"title": title, "description": description}
...
async def test_patch_vacancy_not_found(hr_client):
...
async def test_patch_vacancy_invalid_params(hr_client):
    title = "Vacancy 1"
    description = "Test Description"
    response = await hr_client.post(
        "/vacancies", json={"title": title, "description": description}
...
async def test_filter_vacancies_ok(hr_client):
    ...
    resp2 = await hr_client.post("/vacancies", json=v2_data)
    ...
    # search in title
    response = await hr_client.get("/vacancies", params={"title": "Python"})
    assert response.status_code == 200
    data = response.json()
