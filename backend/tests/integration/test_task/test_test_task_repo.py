...
from app.models.test_task import TestTaskCreate, TestTaskSort, TestTasksFilter
...
async def test_filter_by_vacancy_id(test_task_repo, vacancy_repo):
    v1_id = (await vacancy_repo.create_vacancy(VacancyCreate(title="V1", description="D1")))["id"]
    v2_id = (await vacancy_repo.create_vacancy(VacancyCreate(title="V2", description="D2")))["id"]

    await test_task_repo.create_test_task(TestTaskCreate(title="T1", test_task_url="http://1.com", vacancy_id=v1_id))
    await test_task_repo.create_test_task(TestTaskCreate(title="T2", test_task_url="http://2.com", vacancy_id=v2_id))
...
async def test_filter_by_title_substring(test_task_repo, vacancy_repo):
    ...
    await test_task_repo.create_test_task(TestTaskCreate(title="Python Developer", test_task_url="http://1.com", vacancy_id=v1_id))
    await test_task_repo.create_test_task(TestTaskCreate(title="Frontend Lead", test_task_url="http://2.com", vacancy_id=v2_id))
...
async def test_filter_by_vacancy_title(test_task_repo, vacancy_repo):
    v1_id = (await vacancy_repo.create_vacancy(VacancyCreate(title="Backend Ruby", description="D1")))["id"]
    v2_id = (await vacancy_repo.create_vacancy(VacancyCreate(title="Data Science", description="D2")))["id"]

    await test_task_repo.create_test_task(TestTaskCreate(title="Test 1", test_task_url="http://1.com", vacancy_id=v1_id))
    await test_task_repo.create_test_task(TestTaskCreate(title="Test 2", test_task_url="http://2.com", vacancy_id=v2_id))
...
async def test_sorting_and_pagination(test_task_repo, vacancy_repo):
    v_id = (await vacancy_repo.create_vacancy(VacancyCreate(title="V", description="D")))["id"]

    for t in ["A-task", "B-task", "C-task"]:
        await test_task_repo.create_test_task(TestTaskCreate(title=t, test_task_url="http://test.com", vacancy_id=v_id))

    filters = TestTasksFilter(
        sort_by=TestTaskSort.TITLE,
        sort_order="desc",
        limit=2,
        offset=0
    )
...
async def test_url_contains_filter(test_task_repo, vacancy_repo):
    ...
    await test_task_repo.create_test_task(TestTaskCreate(title="T1", test_task_url="https://github.com", vacancy_id=v_id))
    await test_task_repo.create_test_task(TestTaskCreate(title="T2", test_task_url="https://gitlab.com", vacancy_id=v_id))
...
async def test_no_filter_ok(test_task_repo, vacancy_repo):
    v1_id = (await vacancy_repo.create_vacancy(VacancyCreate(title="V1", description="D2")))["id"]
    v2_id = (await vacancy_repo.create_vacancy(VacancyCreate(title="V1", description="D2")))["id"]

    await test_task_repo.create_test_task(TestTaskCreate(title="T1", test_task_url="https://github.com", vacancy_id=v1_id))
    await test_task_repo.create_test_task(TestTaskCreate(title="T2", test_task_url="https://gitlab.com", vacancy_id=v2_id))
